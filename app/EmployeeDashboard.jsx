...
const EmployeeDashboard = () => {
  ...
  const [timeSheetData, setTimeSheetData] = useState({
   ...
   const mapApiResponse = (data) => ({
    ...
    approve: data.ACCEPTED ?? 0,
    reject: data.REJECTED ?? 0,
    total: data.TOTAL ?? 0,
  });
   ...
   setLoading(true);
    const fetchData = async () => {
      ...
      try {
       ...
       let taskApi =
          role === "employee"
            ? `${url}/apis/employees/TasksDetails/personId/${employeeId}`
            : `${url}/apis/employees/TasksDetails/managerId/${employeeId}`;
      ...
      try {
       ...
       let timeSheetsApi =
          role === "employee"
            ? `${url}/api/timesheet-data/employee-performance-metrics/${employeeId}`
            : `${url}/api/timesheet-data/manager-performance-metrics/${employeeId}`;

        const response = await axios.get(timeSheetsApi, {
          headers: {
            Authorization: `Bearer ${token}`,
            "X-Tenant-ID": id,
          },
        ...
        setTimeSheetData(mapApiResponse(response.data));
        ...
        // let leaveApi=role==="employee"?`${url}/api/leaves/getStatus/${employeeId}/mobile`:`${url}/api/leaves/getStatusForManager/${employeeId}/mobile`;
         let leaveApi =
          role === "employee"
            ...
            : `${url}/api/leaves/getStatusForManager/${employeeId}`;
        const response = await axios.get(leaveApi, {
          headers: {
            Authorization: `Bearer ${token}`,
            "X-Tenant-ID": id,
          },
