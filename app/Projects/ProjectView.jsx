...
const ProjectView = () => {
  ...
  const token = data.token;
    ...
    const handleAddEmployee = (newEmpId) => {
        ...
        if (!isExist) {
            let newMember=projectMembers.filter((each)=>each.employeeId===newEmpId)[0];
             if(!newMember){
                newMember = {
                employeeId: newEmpId,
    ...
    const selectAll = () => {
        ...
        if (projectData.projectMembers.length === allEmployees.length && projectData.projectMembers.length > 0) {
            setProjectData((prev) => ({
                ...prev,
                projectMembers: []
            }))
    ...
    const updateDisplayedPages = (page) => {
        ...
        const filterData = () => {
            let filterByName = allEmployees.filter((each) => (each.firstName + " " + each.lastName).toLowerCase().includes(searchText.toLowerCase()));
            let filterByJobRole = allEmployees.filter((each) => each.jobRole.toLowerCase().includes(searchText.toLowerCase()));
        ...
        if(projectData.projectStatus==="COMPLETED" && projectData.completeDate===null){
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.put(`${url}/api/projects/${id}`, projectData, {
                headers: {
                    Authorization: `Bearer ${token}`,
                    "Content-Type": "application/json",
                    "X-Tenant-ID": schemaName,
...
<View style={styles.inputColumn}>
  ...
  <TextInput
    style={[styles.input, styles.disabledInput]}
    value={projectData.completeDate}
    editable={false}
  ...
  {errors?.completeDate && (
    <Text style={styles.errorText}>{errors.completeDate}</Text>
          ...
          {/* <Text>
            Choose employees and set their roles and status for this project.
          </Text> */}
          <TextInput
            style={[styles.input, styles.disabledInput]}
